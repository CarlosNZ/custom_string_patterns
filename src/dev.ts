...
patternWithLiterals.gen().then((result: string) => console.log('result', result))

const patternWithInCompleteLiterals = new Pattern(/\<1>-[A-Z]{3}-<+dd>-<?f1>/, {
  customReplacers: { f1: () => 'Testing' },
})

patternWithInCompleteLiterals.gen().then((result: string) => console.log('result', result))
